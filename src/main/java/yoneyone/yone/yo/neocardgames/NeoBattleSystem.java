...
public class NeoBattleSystem {
    ...
    //メイン処理
    public void gameMaster(ItemStack itemStack,Player player){
        Bukkit.getScheduler().runTaskAsynchronously(this.neo,() -> {
            ...
            if (card.now) {
                ...
                if (system.getBattle(player) == null){//もしも攻撃処理で対戦が終了していたら停止する
                    return;
                }
                //回復
                if (card.healNow != -1) {
    ...
    //遅延でこのターン発動する場合はこっち
    //メインスレッドで開いてください
    public void gameMaster2(ItemStack itemStack,Player player){
        Bukkit.getScheduler().runTaskAsynchronously(this.neo,() -> {
    ...
    private boolean gameEndCheck(Player player){
        NeoGameSystem system = new NeoGameSystem(neo);
        Battle battle = system.getBattle(player);
        trimHP(battle);//チェック
        int HPA = battle.HPA;
